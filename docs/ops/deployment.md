...
>
> 持久对象 contract revision 2（2026-07-23）采用默认拒绝和硬切换：MinIO 只保存白名单长期
> 业务对象，Runtime run/steps/signals/stdout/state、HTML、普通截图与 raw snapshot 只保留在
> 本地短期 artifact。发布时必须停掉全部旧 worker，执行 migration 后只启动同一 revision。
...
4. `MinIO` 新写入只包含白名单长期业务对象，运行诊断只出现在本地 `artifact_root`。
...
| `MinIO` | 对象存储 | 只保存 contract 白名单长期业务对象 |
...
当前正式对象存储使用 `MinIO`。`run.json`、`steps.json`、`signals.json`、`stdout.log`、
`state.json`、HTML、page/network data、普通诊断截图、飞书 raw snapshot、FastMoss raw response
和临时下载只保留在本地 `artifact_root`。MinIO 新写入只允许
TikTok/Amazon 白名单业务媒体、Amazon `normalized_capture` 和 blocked/captcha/access-blocked
受控终态证据截图，且引用必须包含 `bucket + object_key + content_digest`。生产 worker 使用
预建 bucket，`MINIO_CREATE_BUCKET=false`，不得自行建 bucket。
...
3. 确认 `executor.local.env` 只包含 Runtime / Fact worker URL，migration URL 不在 executor、skill 或 plist 中。
...
9. 读取 Runtime/TikTok 与 `fact_alembic_version` revision，确认它们与应用要求完全一致；仅凭表存在不能通过发布门禁。
10. 使用 Fact worker URL 只读核验角色身份、无高权属性，并扫描所有非系统 schema 的 table、
...
6. 部署/preflight 身份可以只读确认预建 `MinIO` bucket；如已有白名单业务对象，可对已知完整引用执行 stat/read 校验。生产 worker 不创建通用 smoke object，也不因健康检查获得 create bucket/delete 权限；测试写入只能由独立测试身份在显式 test-only namespace 完成并清理。
