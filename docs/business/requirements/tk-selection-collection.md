...
7. 商品链接无效时写入 `商品状态=链接不可访问` 并跳过；商品下架或区域不可售时写入对应状态，跳过 TikTok 媒体采集，但继续 FastMoss 数据和 FastMoss 图表采集。
...
5. `商品状态=链接不可访问` 的记录跳过；`商品状态=已下架/区域不可售` 的记录仅在 FastMoss 必填字段存在空值时进入候选。
...
6. URL 校验通过后进入 TikTok 商品可访问性检查；商品已下架或区域不可售时，回写 `商品状态=已下架/区域不可售`，跳过 TikTok 媒体采集，但继续 FastMoss base、overview、SKU 和 FastMoss 图表采集。
...
4. 商品已下架或区域不可售时，回写 `商品状态=已下架/区域不可售`，跳过 TikTok 媒体采集，但正常采集 FastMoss 数据和 FastMoss 图表。
