...
| --- | --- | --- | --- | --- |
| `TK选品收集` | `TK竞品收集` | `商品ID -> SKU-ID`，`商品链接 -> 产品链接`，`标题 -> 标题` | 高 | 这是同一批商品在“研究池”和“运营池”中的两种形态。当前视图中 5 个有 `商品ID` 的选品商品全部能在竞品表找到。 |
...
- `TK选品收集` 当前视图有 18 条记录，但只有 5 条记录有完整的 `商品ID / 店铺名称 / 标题 / 当前价格 / 评论数 / 评分`。
...
| `标题` | 文本 | 5 | 商品标题，可与竞品表 `标题` 对齐。 |
| `当前价格` | 数字 | 5 | TikTok 商品当前售价。 |
| `评论数` | 数字 | 5 | 评论数量。 |
| `评分` | 数字 | 5 | 商品评分。 |
| `商品主图` | 附件 | 17 | 商品主图素材。 |
| `商品侧边栏图片` | 附件 | 5 | 商品侧边展示图，用于补充视觉信息。 |
| `出单种类占比图` | 附件 | 1 | 订单结构分析截图。 |
| `今年总销量` | 数字 | 5 | 当前商品今年累计销量。 |
| `销量趋势图` | 附件 | 1 | 销量趋势图。 |
| `SKU销量占比图` | 附件 | 1 | 基于 FastMoss SKU 分析有效 `best_sku` 的 SKU 维度销量分析截图。 |
| `父体规格` | 文本 | 1 | FastMoss SKU 分析有效 `best_sku.sku_value` 对应的父体规格。 |
| `父体图片` | 附件 | 1 | 有效 `best_sku` 对应 SKU 图片；无法通过 `sku_id` 或 `prop_value_id` 匹配图片时不写入。 |
...
| `达人联系方式` | 文本 | 1 | FastMoss 达人联系方式；多个联系方式时优先邮箱，否则取第一个有效联系方式。 |
| `检查达人名称是否重复` | 公式 | 81 | 自动判重字段，当前大多返回 `唯一`。 |
| `记录日期` | 日期 | 2 | 达人首次创建或入池日期，原 `记录时间` 字段更名。 |
| `更新日期` | 日期 | 0 | 系统最近一次更新或合并该达人行的日期。 |
