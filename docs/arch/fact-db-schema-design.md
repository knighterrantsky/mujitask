...
Fact DB 可以保存结构化 JSON 扩展字段，但不能承担对象存储职责。`facts_json`、`metadata_json`、`projection_json` 和 raw response 相关 JSON 字段只用于小型结构化事实、证据索引和 schema 演进，不用于长期保存大块 raw payload。

Fact DB JSON 允许保存:

- 已归一化但暂未拆列的业务事实，例如稳定枚举、数值、短文本、小型属性集合。
- raw response 的索引元数据，例如 endpoint、request method、status code、digest、byte size、object key、采集时间和少量排障 preview。
...
- 主体表 `facts_json` / 关系表 `metadata_json` 应保持可索引、可快速读取的小型扩展字段；超过 100KB 必须评估是否应拆表或转 object storage。
- raw response 超过 256KB 时，默认应写入 MinIO / artifact/object storage，Fact DB 只保存 `digest`、`object_key` / `remote_uri`、`byte_size`、`content_type` 和必要 preview。
- 需要长期分析的字段应拆入主体、关系、latest 或 observation 表，不应长期藏在大 JSON 中。

该边界同时约束 Runtime DB: Runtime result 只保存 Fact DB entity key、persisted count、raw/artifact ref 和错误摘要，不复制 Fact DB raw payload 或完整 fact bundle。
...
- 媒体采集输出标准 media asset refs，并通过 `media_asset_sync` materialize 到对象存储。
...
- 如果已有记录包含可用的 `object_key` 或 `remote_uri`，则不重复下载资源图片，只补齐 `tk_entity_media_assets` 绑定。
...
- 小型 raw response 可以直接保存用于排障和回放。
- 大型 raw response 必须先写 MinIO / artifact/object storage，`tk_raw_api_responses` 只保存 digest、object key/URI、byte size、content type、endpoint、采集上下文和必要 preview。
- `fact_bundle_upsert` 的 Runtime result 不返回完整 raw response 或完整 fact bundle，只返回 persisted counts、entity/relation count 和 raw response refs。
- 飞书整表读取结果属于 source snapshot，不属于 Fact DB 主体事实；完整 snapshot 进入 artifact/object storage，Fact DB 最多保存与事实主体相关的 source binding 或 compact projection 摘要。
...
生产发布必须先由 migration user 执行 `20260714_0007`，再启动包含 Amazon handler 的
worker。`ensure_amazon_fact_schema` 只用于本地开发和隔离测试，Store 构造函数、daemon、
dispatcher、watchdog 和 handler 均不得调用它。代码回滚时优先停用 Amazon 入口并保留新增
表；只有完成备份且确认不保留 Amazon 数据后，才 downgrade 到 `20260528_0006`。该
downgrade 只删除 9 张 `amazon_*` 表及其索引，不触碰 TikTok 或 Runtime 表。
