...
| 当前 workflow_id | `tiktok_fastmoss_product_ingest_v1` |
| 顶层表 | `task_request` |
| 编排者 | `executor_daemon` |
...
当前代码中的 `WorkflowSpec` 以 `orchestrate_tiktok_fastmoss_product_ingest` 作为顶层 orchestration step。Runtime 层实际由 executor 根据 `task_request.current_stage` 和 `stage_cursor_json` 派发 API job / browser job。
...
flowchart TD
    ...
    B -->|是| C["Stage 1: feishu_tk_selection_table_read"]
    B -->|否| D["Stage 2: tiktok_product_request_fetch + fastmoss_product_fetch"]
    C --> D
    D --> E{"TikTok request 是否有效?"}
    E -->|是| F["Stage 3: fact_bundle_upsert"]
    E -->|否| G{"是否允许 browser fallback?"}
    G -->|是| H["Stage 2b: tiktok_product_browser_fetch"]
    G -->|否| I["failed / skipped"]
    H --> J["normalize browser result"]
    J --> F
    F --> K{"是否需要写回 TK选品收集?"}
    K -->|是| L["Stage 4: feishu_tk_selection_table_writeback"]
    K -->|否| M["Stage 5: ready_for_summary"]
    L --> M
    M --> N["summary / outbox"]
```
...
| --- | --- | --- | --- | --- |
| `read_selection_table` | 开启 TK selection table mode | 派发飞书读取 job，获取来源记录和写回上下文 | `feishu_tk_selection_table_read` | 读取成功 / 跳过 / 失败 |
...
| `persist_facts` | 已得到 normalized product result | 写商品、店铺、达人、视频、媒体资产、关系和观测 | `fact_bundle_upsert`、可选 `media_asset_sync` | 事实写入完成 / failed |
...
| 当前状态 | 目标语义 |
| --- | --- |
| `waiting_feishu_tk_selection_table_read` | 等待选品表读取 |
...
| `dispatch_feishu_tk_selection_table_writeback` / `waiting_feishu_tk_selection_table_writeback` | 派发或等待选品表写回 |
...
| --- | --- | --- | --- | --- |
| `feishu_tk_selection_table_read` | `api_worker_job` | `api_worker` | `selection_table_read_handler` | source row、product key、writeback context |
| `tiktok_product_request_fetch` | `api_worker_job` | `api_worker` | `tiktok_product_request_fetch_handler` | normalized TikTok product result、fallback decision |
| `fastmoss_product_fetch` | `api_worker_job` | `api_worker` | `fastmoss_product_fetch_handler` | FastMoss 商品、店铺、指标、关联信息 |
| `tiktok_product_browser_fetch` | `task_execution` | `browser_worker` | `tiktok_product_browser_fetch_handler` | browser 原始数据和 normalized product result |
| `media_asset_sync` | `api_worker_job` | `api_worker` | `media_asset_sync_handler` | MinIO object keys、媒体资产事实 |
| `fact_bundle_upsert` | `api_worker_job` | `api_worker` | `fact_bundle_upsert_handler` | entity / relation / observation upsert result |
| `feishu_tk_selection_table_writeback` | `api_worker_job` | `api_worker` | `selection_table_writeback_handler` | 飞书写回结果 |
| `task_completed_notification` | `notification_outbox` | `outbox_dispatcher` | outbox handler | 最终通知 |

## 6. TikTok Request-First / Browser-Fallback 约定
