...
- 父任务 summary 只能汇总最终行级业务终态结果；不能只按 `api_worker_job.status` / `task_execution.status` 汇总，不能看派生 candidate 数量，也不能把 browser request、browser success 或 seed write success 当成详情采集 success。
- Browser 阶段产生的截图、HTML、raw page dump 只允许作为本地短期诊断文件，不得写入 MinIO，也不得成为原行继续执行的跨进程依赖；跨进程只传受控的小型 normalized result。只有商品媒体等 `contracts/facts/durable-business-object-storage.yaml` 白名单中的长期业务对象可以进入 MinIO，并由 `competitor_row_refresh` 内的既有 `media_asset_sync` owner 物化。正式 workflow 需要持久媒体时，缺 Fact DB 或 MinIO 配置必须 fail fast，不能用 `dry_run` 或本地文件成功替代。
...
sequenceDiagram
    ...
    alt browser fallback required
        ...
        Browser->>Local: keep page/network diagnostics locally
...
flowchart TD
    ...
    E --> E1{"row browser request?"}
    E1 -->|是| E2["browser task_execution<br/>+ row job wait resolved"]
    E2 --> E
...
| `refresh_competitor_rows` | 使用与竞品采集一致的 `competitor_row_refresh` 行级 pipeline 补齐详情；row fallback 必须经 browser task_execution 后由原行形成最终行级结果 | `api_worker_job` / `task_execution` |
| `ready_for_summary` | 汇总搜索、种子写入、商品采集和详情写回的最终业务结果；不得存在未处理 wait/fallback 引用或 active browser execution | `task_request` / `notification_outbox` |
...
`fastmoss_product_search` 的响应只允许提取受控尺寸、已脱敏的结构化 `raw_evidence` 写入 Fact DB；额外完整响应只可作本地短期诊断，通用 response body 不进入 MinIO，也不直接进入竞品表 mapper。search 结果已经是 normalized candidates，因此本 workflow 不再单独定义 search result mapper；业务 job 只按返回顺序逐条调用种子写入。种子写入的已存在判断沿用 `competitor_seed_projection_mapper` 输出的 `upsert_key`: 优先使用 `SKU-ID` / `product_id`，缺少 product_id 时才按标准化 `产品链接` 兜底。
...
同一能力也服务搜索后的详情补齐和其它 FastMoss provider API。`fastmoss_product_fetch`、`fastmoss_creator_fetch`、`fastmoss_shop_fetch`、`fastmoss_video_fetch` 遇到 `MSG_SAFE_0001` 时，API handler 必须返回业务化 browser request 和脱敏 `verification_request`，其中包含原始失败的 FastMoss API 请求 method/path/params/referer/region/stage，例如商品详情 `/api/goods/v3/base`。executor/runtime 基于唯一 waiting row job 派发 `fastmoss_security_browser_resolve`，browser handler 只验证该原始失败请求不再返回 `MSG_SAFE_0001`，成功后持久化 FastMoss cookies，并由 executor/runtime 写回脱敏 metadata 后让原调用方重试原 handler 一次。
...
- 审计必须记录 ddddocr 原始坐标、坐标换算、拖动距离、前后截图、目标位置截图、失败态文本、滑动后轮询结果和二次确认结果，便于区分识别错误、距离计算错误和鼠标执行异常。目标位置截图 artifact 名称为 `target_position_screenshot`，必须在鼠标移动到计算终点且释放之前捕获；这些验证码诊断文件只保留在本地 `artifact_root`，不得进入 MinIO。
...
- `keyword_seed_import` 的 FastMoss search fallback: 业务化 browser request 表示等待 `fastmoss_security_browser_resolve` 或等待原始 `/api/goods/V2/search` 重试。browser success 只能解除阻塞并重试原搜索，不能直接把关键词任务计为 success。
- 详情补齐阶段的 row fallback: 新增成功的竞品种子行进入 `competitor_row_refresh` 后，TikTok / FastMoss fallback 必须按第 3.2.1 节由原行级 pipeline 消费或重试原 stage。seed write success 只表示种子行创建成功，不等于详情采集 success。

`ready_for_summary` 入口条件是: `keyword_seed_import` 已终态、所有成功 seed row 都有最终行级业务结果、且不存在 waiting row job、active browser execution 或未收敛行级主 job。
...
sequenceDiagram
    ...
    alt MSG_SAFE_0001 after one login refresh retry
        API->>DB: mark keyword_seed_import waiting with original search request context
...
- 父 task 基于最终业务结果汇总：关键词种子导入结果 + 每条成功 seed row 的最终行级结果。browser request、browser child success、seed write success 都不能替代详情行终态。
