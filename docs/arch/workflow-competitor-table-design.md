...
TikTok request 必须实际发起并写入 attempt 证据。只有返回明确风控、登录、验证码、访问受限或缺少商品详情脚本时，`competitor_row_refresh` 才能返回 wait/fallback signal；兼容期可带 `fallback_required`。workflow 派发 `tiktok_product_browser_fetch` 子执行，并把当前行级主 job / request 置为 `status=waiting`。商品不可访问、已下架或区域不可售是 request 阶段可判定的终态，应直接写回 `商品状态=已下架/区域不可售` 并停止该行后续 browser fallback、媒体同步和 FastMoss 补齐；普通网络失败、超时、5xx、429 或代理临时异常先按 retry policy 重试，不能直接 fallback。
...
- `fallback_required` 是兼容期 handler wait signal，不是 Runtime DB `status`，不是行级终态，不能计入父任务 success，也不能让 `ready_for_summary` 提前开始。
- TikTok browser fallback 属于替代当前 TikTok 采集 stage 输出。Browser `task_execution` success 只证明浏览器能力完成，不等于竞品行刷新完成；原 `competitor_row_refresh` 必须消费 browser normalized result 引用，继续 media sync、FastMoss、Fact DB、Feishu writeback，并产出最终行级状态。
- FastMoss security fallback 属于解除阻塞后重跑当前 FastMoss stage。Browser handler 成功后直接持久化 `fastmoss_session_cookie_cache` 并返回脱敏 metadata；原 FastMoss handler 读取 cookie cache 后重试原 API 一次，不通过 payload 传递 cookie value。
...
- 父任务 summary 只能汇总最终行级业务结果；不能只按 `api_worker_job.status` / `task_execution.status` 汇总，也不能把 wait/fallback signal、browser success 或 seed write success 当成详情采集 success。
- Browser 阶段产生的截图、HTML、raw page dump 等物理资产可以在 browser handler 内先写对象存储并由 `artifact_object` 索引；商品媒体资产、Fact DB upsert 和 Feishu projection 的最终一致性仍由 `competitor_row_refresh` 行级主 job 承担。正式 workflow 缺 Fact DB 或 MinIO/S3 对象存储配置时必须 fail fast，不能用 `dry_run` 或 `local` 成功替代。
...
- `keyword_seed_import` 的 FastMoss search fallback: wait/fallback signal 表示等待 `fastmoss_security_browser_resolve` 或等待原始 `/api/goods/V2/search` 重试。browser success 只能解除阻塞并重试原搜索，不能直接把关键词任务计为 success。
...
`ready_for_summary` 入口条件是: `keyword_seed_import` 已终态、所有成功 seed row 都有最终行级业务结果、且不存在未处理的 wait/fallback 引用、active browser execution 或未收敛行级主 job。
...
- 父 task 基于最终业务结果汇总：关键词种子导入结果 + 每条成功 seed row 的最终行级结果。wait/fallback signal、browser child success、seed write success 都不能替代详情行终态。
