...
- 当 stage 需要等待 child job、browser fallback 或外部可观测事件时，把 `task_request` 或触发的行级 `api_worker_job` 标记为 `status=waiting`，并在 `result_json.wait` 记录被等待对象引用、来源 stage、来源 job、fallback handler 和 workflow 定义的恢复策略。
- 当 Reconciler 判断等待条件解除时，把顶层请求改回 `status=pending` 并保留或推进 `current_stage`，例如 `current_stage=ready_for_summary`。
- 只汇总终态结果、写 summary/outbox，不直接执行 handler、不解析 browser 页面数据、不写业务字段 mapper。
...
- 遇到需要 browser fallback 的业务场景时，只能返回标准等待/失败结果或写入等待上下文；不得内联执行 browser handler，也不得把 `fallback_required` 写成 Runtime DB lifecycle status。
...
- 对 `status=waiting` 的 parent 或 row job，检查 `result_json.wait` 中引用的 child 是否全部终态。
- child 仍有 `pending/running/waiting` 时保持 parent `waiting`。
- child 全部终态时，按 workflow contract 推进: TikTok browser fetch 可把 browser result 作为当前 stage 输出引用；FastMoss security resolve 只解除阻塞并让原 FastMoss handler/stage 重试一次。
