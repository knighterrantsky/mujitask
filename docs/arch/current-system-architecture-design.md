...
1. Agent Artifact / Entry Layer: 目标归属 `agent_artifacts/**` 和 `apps/**`，包含 OpenClaw / Hermes / 用户 agent workspace 的 skill/script，以及 RPC Agent Service、CLI。
2. Runtime Control Plane: 目标归属 `control_plane/**`，包含 task request、executor、daemon、Execution Supervisor、Reconciler、Watchdog、outbox、runtime config。
3. Domain Orchestration Layer: 目标归属 `domains/**`，包含业务 task、workflow、job、policy、mapper、projection、flow。
4. Capability Layer: 目标归属 `capabilities/**`，包含可复用 input source、fact source、persistence、channel、browser、media handler。
5. Infrastructure Layer: 目标归属 `infrastructure/**`，包含外部系统 client、Runtime Store、Fact Store、Object Store、schema/migration、observability。
6. Deployment / Configuration Layer: 目标归属 `configs/**` 和 `scripts/**`，包含项目配置、runtime 配置、agent 配置、launchd/systemd/docker/k8s 部署配置。

当前代码还没有完全迁移到目标目录结构，但系统职责必须按上述分层治理。新增业务或重构时，应以 [目标项目架构契约](./target-project-architecture-contract.md) 为目标，以 [项目结构与命名契约](./project-structure-contract.md) 记录当前落位，以 [Runtime 控制面契约](./runtime-control-plane-contract.md) 约束运行控制入口。
...
flowchart TD
    subgraph L0["Layer 0: Agent Artifact / Entry"]
        Skill["agent_artifacts / skills<br/>OpenClaw / Hermes / 用户 agent workspace"]
...
| 目标层 | 当前落位 | 当前职责 | 不应放入 |
| --- | --- | --- | --- |
| Agent Artifact / Entry | `skills/mujitask-tiktok-feishu-sync/`、`agent.py`、`cli.py` | Agent skill bundle、RPC task registry、本地 submit/status/debug | workflow 主编排、worker loop、字段 mapper |
| Runtime Control Plane | `business/flows/runtime_orchestrator.py`、`runtime_common.py`、`execution_supervisor.py`、`runtime_views.py`、`watchdog_scanner.py`、根包 daemon 文件、`project_env.py`、`config.py` | task request 生命周期、executor、worker claim、supervisor、reconciler、watchdog、outbox、runtime config | Feishu 表字段、TikTok/FastMoss 业务策略、业务专用 daemon |
| Domain Orchestration | `business/tasks/`、`business/workflow_defs/`、`business/jobs/`、`business/feishu/`、`business/flows/runtime_*`、`business/flows/achieve/**` | task/workflow/job、业务 mapper、projection、policy、业务组合 flow | 外部系统底层 client、Runtime DB store、daemon main |
| Capability | `business/handlers/api/`、`business/handlers/browser/`、`business/handlers/outbox/`、handler registry/contract | 通用 worker 能力: 输入源读取、事实采集、事实入库、媒体同步、飞书写回、outbox 分发、浏览器采集 | 单个业务的字段筛选、业务投影、终态汇总 |
...
当前目录和目标目录不一一相同，但职责必须先按目标分层判断。迁移前的代码如果仍位于 `business/flows/**`，也要明确它属于 Domain Orchestration 还是 Runtime Control Plane，不能继续混用。
...
- skill bundle 放在 `agent_artifacts/skills/{skill_code}` 的目标层；当前兼容落位是 `skills/{skill_code}`。
...
| 组件 | 当前入口 | 职责 |
| --- | --- | --- |
| RPC Agent Service | `agent.py` | 暴露 framework/platform 兼容 task registry 和 submit 入口 |
| CLI | `cli.py` | 本地 submit/status/debug/list-tasks |
| Task Request Entry | `business/tasks/**`、`runtime_orchestrator.py::submit_task_request` | 创建顶层请求、查询状态和结果 |
| Executor | `executor_daemon.py`、`runtime_orchestrator.py` | claim task_request、读取 workflow、拆 job、推进 stage、写 summary/outbox |
| API Worker Daemon | `api_worker_daemon.py` | 消费 API/IO 类 job，交给 supervisor 和 handler registry |
| Browser Runloop | `browser_runloop.py` | 串行消费浏览器/profile/CDP 类任务 |
| Execution Supervisor | `execution_supervisor.py` | heartbeat、progress、timeout、异常归一化、result/error envelope |
| Reconciler | `runtime_views.py`、`runtime_orchestrator.py` 的汇总路径 | 汇总 child job 终态，推动 parent request 进入下一阶段或 ready_for_summary |
| Watchdog | `watchdog_scanner.py`、`business/flows/watchdog_scanner.py` | 修复 lease expired、stale progress、execution timeout、stuck parent、outbox timeout |
| Outbox Dispatcher | `outbox_dispatcher.py`、outbox handler | claim `notification_outbox`、发送消息、处理通道重试 |
| Runtime Config | `project_env.py`、`config.py`、`scripts/execution_control/executor.local.env` | 配置加载优先级、typed defaults、runtime/agent/deployment 配置边界 |
...
| Input Sources / Dingding Sheet | 目标扩展 | 未来钉钉表格读取应与飞书同类，不能写成业务专用 handler |
...
| Fact Sources / AWS | 目标扩展 | AWS 作为事实源或对象源时放 fact source / persistence capability |
...
| Channels / Dingding / Discord | 目标扩展 | 出站消息必须经过 outbox |
...
| 问题 | 应落位 |
| --- | --- |
| 这个业务是否有 agent 入口，skill 如何部署到用户 agent workspace? | `agent_artifacts/skills/{skill_code}`，当前兼容 `skills/{skill_code}` |
| 顶层业务入口是什么，参数和回执是什么? | `domains/{domain}/tasks/{task_code}`，当前兼容 `business/tasks` |
| workflow 分几阶段，每阶段依赖什么 job? | `domains/{domain}/workflows/{workflow_code}`，当前兼容 `business/workflow_defs` |
| 每个 job 绑定哪个通用 handler capability? | `domains/{domain}/jobs/{job_code}`，当前兼容 `business/jobs` |
