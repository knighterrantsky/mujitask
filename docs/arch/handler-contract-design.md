...
| API | `selection_row_refresh` | `api_worker` | `api_worker_job` | Business | TK 选品表行级 pipeline 主 job；每条候选行 1 个，内部串行完成 TikTok 采集 → 媒体同步 → FastMoss 采集 → Fact DB upsert → 飞书必填补全字段、系统运行字段和可选补充字段写回 | workflow-selection-table-expand-design 4 |
