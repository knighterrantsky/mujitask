...
`raw_snapshot_ref` 指向的 artifact 内保存 `"raw_rows"` 和 `"raw_rows_all"`；Runtime result 默认只保存以下紧凑引用和候选行。
...
```json
{
  "source_rows": [
    {
      ...
      "source_snapshot_ref": "s3://runtime-artifacts/feishu/competitor/read/req-001/page-1.json"
    }
  ],
  "schema": {
    "field_names": ["产品链接", "SKU-ID", "商品状态", "达人查找状态", "备注"]
  ...
  "raw_snapshot_ref": "s3://runtime-artifacts/feishu/competitor/read/req-001/page-1.json",
  "raw_snapshot_artifacts": [
    {
      "kind": "feishu_table_read_raw_json",
      "object_key": "runs/run-001/artifacts/feishu/competitor/read/page-1.json",
      "size": 4096,
...
| `raw_rows` | 飞书传输层原始标准化行，只允许写入 `raw_snapshot_ref` 指向的 artifact；Runtime result 默认不得内联，只有显式 `include_raw_rows_in_result` 调试开关可临时返回。 |
...
- `tiktok_product_browser_fetch` 持久化它实际采集到的浏览器资产: screenshot、HTML、network/raw page dump 等大对象写入对象存储并由 `artifact_object` 索引；小型 normalized product result、quality/missing 字段和 artifact refs 写入 `task_execution.result_json`。
- TikTok browser fallback 属于“替代当前 stage 输出”的 fallback。browser 成功后，executor/runtime 把 `task_execution.result_json.normalized_product_result` 的引用写回原 row job，行级主 job 用该引用继续 media sync、Fact DB upsert 和飞书 projection；不需要把大对象复制回原 job payload。
...
- Fact handler 在任何事实写入前读取全部 raw refs，校验实际字节 SHA-256、受控 bucket、
  ASIN、采集日期、`request_id`、同源 Browser `execution_id`、`run_id` 和固定文件名；HTML
  还必须解压后重新通过脱敏检查。
...
- 缺 Fact DB、非本地对象存储、bucket 或 browser profile 时为不可降级配置失败。
