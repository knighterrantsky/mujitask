...
`fallback_required` 是现有 handler 兼容信号，不是 Runtime DB lifecycle `status`，也不是终态 `result_status`。新 contract 中应把 fallback 表达为“当前 job 需要等待 browser 或等待原 stage 重试”的 wait/fallback signal，例如:
...
在迁移期，已有 handler 仍可返回 `status=fallback_required` 或 `result.fallback_required=true`，但 executor/supervisor 必须将其映射为 Runtime `status=waiting` + 等待引用，不能写入 `api_worker_job.status=fallback_required`，也不能把它计入 summary 成功/失败终态。
...
| fallback | request 不可解析、关键字段缺失、风控阻断时返回 wait/fallback signal；兼容期可带 `fallback_required=true` |
...
- TikTok browser fallback 属于“替代当前 stage 输出”的 fallback。browser 成功后，workflow / 行级主 job 可以直接把 `task_execution.result_json.normalized_product_result` 作为当前 TikTok 采集阶段输出引用，继续 media sync、Fact DB upsert 和飞书 projection；不需要把大对象复制回原 job payload。
...
- 只能由 workflow 或行级主 job 在 FastMoss API handler 返回 wait/fallback signal 后派发，API worker 不直接驱动浏览器。
...
- `fastmoss_security_browser_resolve` 属于“解除阻塞后重跑当前 stage”的 fallback。browser handler 成功后直接把 cookie value 写入 `fastmoss_session_cookie_cache`；`task_execution.result_json` 只返回 `cache_key`、`cookie_count`、`has_fd_tk`、`fd_tk_digest`、`verified_path`、`original_request_digest` 等脱敏 metadata。
...
- 当 FastMoss 商品详情接口例如 `/api/goods/v3/base` 返回 `MSG_SAFE_0001` 时，handler 返回 wait/fallback signal；兼容期可继续返回 `fallback_required`。error_code 固定为 `fastmoss_security_verification_required`，result.reason 固定为 `fastmoss_api_security_verification`。
...
- `fastmoss_creator_fetch`、`fastmoss_shop_fetch`、`fastmoss_video_fetch` 与 `fastmoss_product_fetch` 使用同一 FastMoss provider 级风控契约。对应 API 返回 `MSG_SAFE_0001` 时，handler 返回 wait/fallback signal 和脱敏 `verification_request`，由 `fastmoss_security_browser_resolve` 验证原始失败的 FastMoss API 请求并刷新 cookie cache。
