...
| Business Handler Candidate | `fastmoss_echarts_render`、`business_snapshot_write`、`product_candidate_filter` | 否，业务定制 | 进入第 9 节准入清单后才可以 |
...
通用 handler terminal `status`:

| status | 说明 |
| --- | --- |
| `success` | handler 完成，result 可被后续 stage 消费 |
| `skipped` | 输入合法但业务判断无需执行 |
| `partial_success` | 终态业务结果；主输出可用，但部分可选能力失败 |
| `failed` | 当前 job 失败，按 retry policy 判断是否重试 |

`cancelled` 不是 handler 业务结果。取消属于 Runtime 生命周期，由控制面把对应 `task_request` / `api_worker_job` / `task_execution` 标记为 `status=cancelled`，并记录取消原因。

`fallback_required` 是现有 handler 兼容信号，不是 Runtime DB lifecycle `status`，也不是终态 `result_status`。新 contract 中应把 fallback 表达为“当前 job 需要某个 browser handler 处理这件事”的业务化请求。handler 只返回 browser handler code 和业务 payload，不返回 `fallback_source_job_id`、`after_browser_candidates`、恢复 stage 或运行时关联字段；这些关联、调度和 browser result 写回都属于 executor/runtime 层。
...
在迁移期，已有 handler 仍可返回 `status=fallback_required` 或 `result.fallback_required=true`，但 executor/supervisor 必须将其映射为原 row job `status=waiting` + browser `task_execution`。waiting row job 是唯一 fallback 待处理事实，task_execution 是浏览器任务事实；summary gate 只看终态业务结果，不能把 fallback signal 或 browser task success 计入行级成功/失败终态。
...
- 例如 `orchestrate_sync_tk_influencer_pool` 不得出现在目标 handler contract 或 handler registry 中。
...
`"raw_rows"` / `"raw_rows_all"` 只在 `feishu_table_read` 当前进程内供 adapter 使用，不进入 Runtime
result，也不上传 MinIO。result 只保存裁剪后的候选行和计数。
...
P0 冻结以下边界。2026-07-23 的 Storage contract revision 2 只收紧 raw snapshot 字段；
其余字段语义保持兼容。旧 inflight result 中的 `raw_snapshot_ref` / `source_snapshot_ref` 允许被
读取并忽略，新 job 不再生成这些字段:
...
| `raw_rows` | 飞书传输层原始标准化行，只在当前 handler 进程内供 adapter 使用；不得进入 Runtime result、Fact DB 或 MinIO。调试需要时只能写本地短期文件。 |
| `source_rows` | 当 `adapter_code` 存在时输出的业务候选行；adapter 只做字段解析、筛选、去重、writeback context 组装，不写外部系统，且 `source_fields` 必须是紧凑投影，不能携带完整附件/raw/blob。 |
| `raw_snapshot_ref` | revision 1 历史兼容字段；新输出禁止生成，不得作为跨进程输入。 |
| `source_snapshot_ref` | revision 1 历史兼容字段；新输出禁止生成，行级排障使用本地短期诊断。 |
| `empty_row_records` | Runtime 内保留的空行清理线索，只包含 record id 等小字段，不能用 full raw rows 替代。 |
...
| output | normalized product result、受控尺寸 `raw_evidence`、quality_score、missing_fields |
| fallback | request 不可解析、关键字段缺失、风控阻断时返回业务化 browser request；兼容期可带 `fallback_required=true` |
...
| input | `product_url`、browser profile / resource_code；runtime 关联由 executor/runtime 根据唯一 waiting row job 补齐 |
| output | bounded normalized product result；本地诊断索引不得成为业务输入 |
| idempotency | request_id + normalized product url |
| side effects | 本地 browser artifact；长期商品媒体仍由后续 `media_asset_sync` 处理 |
...
- `tiktok_product_browser_fetch` 将 screenshot、HTML、network/raw page dump 保存在本地短期
  `artifact_root`；这些文件不得进入 MinIO 或成为下游业务输入。小型 normalized product result
  与 quality/missing 字段经过 Runtime allowlist 后写入 `task_execution.result_json`。
- TikTok browser fallback 属于“替代当前 stage 输出”的 fallback。browser 成功后，executor/runtime 把受控 `normalized_product_result` 写回原 row job，行级主 job 用该结构化结果继续 media sync、Fact DB upsert 和飞书 projection；不传递或上传诊断大对象。
- 如果 browser 未采集到足够的 normalized product result，当前行按业务规则终态为 `failed` 或允许的 `partial_success`，row cursor 才能放行下一行。
...
- browser fallback 结果必须保留 `slider_captcha_resolution` 与本地
  `slider_captcha_audit_artifact_refs`，用于当次审计 ddddocr 原始坐标、浏览器渲染坐标、
  缩放换算、拖动距离、前后截图和原始图片；这些 ref 不得作为跨进程业务输入或 MinIO 引用。
...
| worker | `browser_worker` |
| runtime table | `task_execution` |
| input | `verification_request` 原始失败 FastMoss API 请求、FastMoss browser profile / resource_code；runtime 关联由 executor/runtime 根据唯一 waiting row job 补齐 |
...
- 只能由 executor/runtime 在 FastMoss API handler 返回业务化 browser request 后派发，API worker 不直接驱动浏览器。
...
- `fastmoss_security_browser_resolve` 属于“解除阻塞后重跑当前 stage”的 fallback。browser handler 成功后直接把 cookie value 写入 `fastmoss_session_cookie_cache`；`task_execution.result_json` 只返回 `cache_key`、`cookie_count`、`has_fd_tk`、`fd_tk_digest`、`verified_path`、`original_request_digest` 等脱敏 metadata。executor/runtime 将这些脱敏 metadata 写回原 row job 后，原 FastMoss stage 才重新读取 cookie cache 并重试原请求。
- 原 FastMoss API handler 不通过 payload 接收 cookie value。它在重试同一原始请求时由 `infrastructure/fastmoss` 读取 cookie cache；如果再次返回 `MSG_SAFE_0001`，当前行或搜索 job 终态为 `failed`，错误码为 `fastmoss_security_verification_required`。
...
- `fastmoss_product_fetch`、`fastmoss_creator_fetch`、`fastmoss_shop_fetch`、`fastmoss_video_fetch` 遇到 `MSG_SAFE_0001` 时，必须返回业务化 browser request；兼容期可继续带 `status=fallback_required` 或 `result.fallback_required=true`。result 中必须给出脱敏 `verification_request.method/path/params/referer/region/stage`；不得把该场景归类为普通 `fastmoss_http_failure`。
...
定位: FastMoss 商品搜索通用能力，通过 `/api/goods/V2/search` 或后续等价搜索 endpoint，根据 keyword、region、排序、分页、输入 filter 和输出 condition 搜索候选商品。关键词搜索竞品写入只是该 handler 的一个调用场景。
...
- 原始响应只允许转换为受控尺寸的结构化 `raw_evidence`，由同一事实写入边界写入 Fact DB；通用 raw response 不上传 MinIO。`raw_response_ref` / `raw_item_ref` 不能作为跨进程输入或远端对象引用，旧值不执行兼容读取、修复、回填或迁移。
...
| output | normalized candidate products、condition evaluation、受控尺寸 `raw_evidence`、pagination result、auth/degraded state |
...
```json
{
  ...
  "raw_capture_policy": {
    "persist_bounded_raw_evidence": true,
    "store_full_raw_response": false,
...
| `raw_capture_policy` | 否 | 受控 raw evidence 写入 Fact DB 的策略；完整响应和单项 raw body 不进入 MinIO |
...
| output | normalized product facts、shop facts、metrics、受控尺寸 `raw_evidence` |
...
- 当 FastMoss 商品详情接口例如 `/api/goods/v3/base` 返回 `MSG_SAFE_0001` 时，handler 返回业务化 browser request；兼容期可继续返回 `fallback_required`。error_code 固定为 `fastmoss_security_verification_required`，result.reason 固定为 `fastmoss_api_security_verification`。
...
达人同步、竞品采集、选品采集和关键词搜索写入流程可以按需消费这些 facts，再由业务 mapper 决定写哪些关系和投影。

风控约束:

- `fastmoss_creator_fetch`、`fastmoss_shop_fetch`、`fastmoss_video_fetch` 与 `fastmoss_product_fetch` 使用同一 FastMoss provider 级风控契约。对应 API 返回 `MSG_SAFE_0001` 时，handler 返回业务化 browser request 和脱敏 `verification_request`，由 `fastmoss_security_browser_resolve` 验证原始失败的 FastMoss API 请求并刷新 cookie cache。
...
```json
{
  ...
  "raw_evidence": [
    {
      "source_endpoint": "author.base_info",
      "content_digest": "sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
    },
    {
      "source_endpoint": "author.contact",
      "content_digest": "sha256:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789"
    }
  ],
  "quality": {
...
定位: 使用本地临时文件下载/转换图片、头像、封面等媒体，并把机器契约准入的长期业务媒体
物化到 MinIO 后写入媒体事实索引。
...
| 项 | 约定 |
| --- | --- |
| input | source_url / file_token / current-process local_path / desired allowlisted object prefix |
| output | asset_key、bucket、object_key、content_digest、content_type、size |
| idempotency | asset_key 或稳定 object_key |
| side effects | MinIO 长期业务媒体、`tk_media_assets` / media relation index |
...
- `raw_evidence` 只承载受控尺寸的结构化证据，由 `fact_bundle_upsert` 写入 Fact DB；写入后返回 `raw_response_ids`。通用 raw response、旧 `raw_refs` 和本地 artifact 都不得成为 MinIO 对象或跨进程业务输入。
...
`feishu_table_read` 和 `feishu_table_write`。Browser handler 成功时只采集并写 normalized
capture，blocked/captcha 时只额外写受控截图；Fact
...
| `amazon_product_browser_fetch` | `browser_worker` / `task_execution` | `requested_asin`、`source_record_id`、`run_id` | identity、collection status、coverage、`normalized_capture_ref`、blocked evidence ref、media source refs | Chrome/指纹浏览器访问；MinIO 只写 normalized capture 或 blocked 受控截图 |
| `amazon_product_fact_upsert` | `api_worker` / `api_worker_job` | normalized capture ref、source table identity、source record、ASIN、run | product/snapshot/binding/normalized capture ids、persisted counts | MinIO 读取、独立 `amazon_*` Fact 写入 |
...
- Browser 成功只持久化 content-addressed normalized JSON；HTML、page-data、network data
  和普通截图只在本地参与解析，不产生 MinIO ref。网络候选必须通过 resolved ASIN 绑定和字段
  allowlist 后才能合并进 normalized capture。
- Fact handler 在任何事实写入前读取 normalized capture ref，校验实际字节 SHA-256、受控
  bucket、ASIN、采集日期、`request_id`、同源 Browser `execution_id`、`run_id` 和固定文件名；
  不读取 Browser 本地路径。
- normalized capture 的 field evidence 必须精确覆盖目标路径，强制携带来源和置信度，
  且 evidence value 与 capture 字段一致；Amazon 图片只允许受控 HTTPS CDN，URL 的 query
  和 fragment 在进入 capture、媒体同步与 Fact 边界前删除。
- `amazon_product_row_persist` 只能通过 registry dispatch 调用 capability handler，不直接
  import 其实现；同一 run 重试复用 snapshot 和对象坐标。
- 缺 Fact DB、MinIO、bucket 或 browser profile 时为不可降级配置失败。
...
| `product_video_outreach_check` | 每个商品 1 个建联视频索引 job，调用 FastMoss 商品视频列表并沉淀 `tk_videos` 和 `tk_video_product_relations`；Runtime result 不承载完整视频原始响应 | 准入；用于 `tiktok_influencer_outreach_sync.index_product_videos` |
| `outreach_creator_video_metric_refresh` | 每个 SKU + 达人行 1 个指标刷新 job，查询全部已知视频，逐条调用 video overview，写 `tk_video_metric_snapshots` 后聚合并写回飞书行 | 准入；用于 `tiktok_influencer_outreach_sync.refresh_creator_video_metrics_and_writeback` |
...
| API | `product_video_outreach_check` | `api_worker` | `api_worker_job` | Business | 达人建联商品视频索引 job；每个商品 1 个，内部调用 FastMoss 商品视频列表并沉淀商品-视频索引 | workflow-influencer-outreach-design 6.3 |
| API | `outreach_creator_video_metric_refresh` | `api_worker` | `api_worker_job` | Business | 达人建联 SKU+达人指标刷新 job；逐条视频采集 overview、写视频指标快照、聚合并写回飞书行 | workflow-influencer-outreach-design 6.5 |
...
| API | `media_asset_sync` | `api_worker` | `api_worker_job` | Capability | 仅把白名单中的图片、头像、封面等长期业务媒体写入 MinIO，并返回完整 `bucket + object_key + content_digest` 供事实索引 | 6.6 |
| API | `fact_bundle_upsert` | `api_worker` | `api_worker_job` | Capability | normalized entities / relations / observations 统一写入 Fact DB | 6.7 |
| API | `selection_row_refresh` | `api_worker` | `api_worker_job` | Business | TK 选品表行级 pipeline 主 job；每条候选行 1 个，内部串行完成 TikTok 采集 → 媒体同步 → FastMoss 采集 → Fact DB upsert → 飞书必填补全字段、系统运行字段和可选补充字段写回 | workflow-selection-table-design 4 |
...
| `orchestrate_sync_tk_influencer_pool` | 禁止准入 | Workflow 编排入口不得注册为目标 handler |
...
| --- | --- | --- |
| `fastmoss_echarts_render` | 生成 ECharts option / HTML / 图片 artifact | artifact schema、输入 facts 范围、重试/超时/幂等规则；HTML 只留本地，图片只有在显式长期业务附件 contract、prefix 与 retention 均已声明时才允许进入 MinIO |
