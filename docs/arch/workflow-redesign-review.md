...
| Workflow | task_code | target workflow_code | 当前业务定位 |
| --- | --- | --- | --- |
...
说明: 当前代码中的历史 `WorkflowSpec` ID 可以保留为 framework 兼容实现事实；目标 Runtime workflow contract 使用稳定 `workflow_code`，不在 `workflow_code`、`stage_code`、`job_code` 或 `handler_code` 中追加版本后缀。
...
- 部分当前实现仍通过 framework 兼容入口进入 Runtime 推进；目标架构不把这些兼容入口作为 workflow contract。
...
本节给出重构后的目标映射。它是代码落地时的主要对照表，比后面各 workflow 的局部说明更接近实现清单。
...
- 旧的 leaf task / 业务专用 handler 不进入目标架构。现有 workflow 重构时直接切到本表定义的通用 handler 链路。

#### 4.6.1 `refresh_current_competitor_table`

目标是刷新已有 `TK竞品收集` 记录。重构后直接拆成“飞书读取 -> 通用商品事实采集 -> 事实入库 -> 竞品表投影写回”，不再保留单行补全业务专用 handler。

| Stage code | Job code | Runtime 表 | Worker / 进程 | Handler code | Adapter / Mapper / Flow | Result 消费方 |
| --- | --- | --- | --- | --- | --- | --- |
| `read_competitor_rows` | `feishu_table_read` | `api_worker_job` | `api_worker` | `feishu_table_read` | `competitor_table_source_adapter` | executor fan-out target rows |
...
| `persist_facts` | `fact_bundle_upsert` | `api_worker_job` | `api_worker` | `fact_bundle_upsert` | `competitor_fact_relation_mapper` | writeback projection |
...
目标是按关键词或其他 filter 在 FastMoss 中搜索候选商品，先写入 `TK竞品收集` 种子行，再复用竞品表刷新链路完成事实采集和投影写回。
...
目标是从 `TK竞品收集` 候选商品出发，发现关联达人，采集达人详情，写入 `TK达人池`，并回写竞品表达人查找状态。
...
| `collect_creator_detail` | `fastmoss_creator_fetch` | `api_worker_job` | `api_worker` | `fastmoss_creator_fetch` | `creator_fact_mapper` + optional `media_asset_sync` | Feishu influencer projection |
...
- creator detail job 如果同时推进达人详情采集和飞书写入，必须在 job checkpoint 中明确幂等边界；目标 handler 仍然使用通用能力 handler。

#### 4.6.4 `tiktok_fastmoss_product_ingest`

目标是围绕单个 TikTok 商品完成 request-first/browser-fallback 采集、FastMoss 商品采集、事实入库、媒体同步，以及可选 `TK选品收集` 投影写回。
...
| `persist_facts` | `fact_bundle_upsert` | `api_worker_job` | `api_worker` | `fact_bundle_upsert` | `selection_fact_relation_mapper` | writeback projection / summary |
...
| `read_competitor_rows` | 有表 URL / view / filter | 读取候选行并标准化 source snapshot | `feishu_table_read` | 得到 target rows |
| `dispatch_product_collection` | 有 target rows | 为每行派发商品采集 job | `tiktok_product_request_fetch`、`fastmoss_product_fetch` | 商品采集 jobs 已创建或跳过 |
...
| `feishu_table_write` | `api_worker_job` | `api_worker` | `feishu_table_write` | target table + product_id / normalized url |
| `tiktok_product_request_fetch` | `api_worker_job` | `api_worker` | `tiktok_product_request_fetch` | product_id / normalized url |
| `fastmoss_product_fetch` | `api_worker_job` | `api_worker` | `fastmoss_product_fetch` | product_id / fastmoss product key |
| `tiktok_product_browser_fetch` | `task_execution` | `browser_worker` | `tiktok_product_browser_fetch` | request_id + product url |
| `media_asset_sync` | `api_worker_job` | `api_worker` | `media_asset_sync` | entity key + asset source |
...
| 问题 | 影响 | 建议 |
| --- | --- | --- |
| framework 兼容入口与 Runtime stage 混在一起 | 外部看不到真实 stage | 内部 `WorkflowDefinition` 显式定义 stage，兼容入口不进入目标 contract |
...
- 文档和代码中不再出现业务专用飞书写入 handler 作为目标路径。
