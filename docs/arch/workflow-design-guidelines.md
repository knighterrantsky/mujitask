...
- Runtime workflow 文档只记录稳定 `workflow_code`；非正式入口 ID 不作为架构设计事实或对外名称。
...
- Browser fallback 是物理上的独立 `task_execution`，但在业务语义上仍属于触发它的当前实体 / 当前行 pipeline。`fallback_required` 是兼容期 handler wait signal，不能作为 Runtime DB `status`，不能作为行级成功、父任务成功或 summary 终态计数。
- 对 `row_pipeline_concurrency=1` 的 workflow，waiting row job 是唯一 fallback 待处理事实，`task_execution` 是浏览器任务事实。同一个顶层请求同一时刻最多只能有一个 waiting row job 和一个未终态 browser `task_execution`。
- executor 负责调度和数据交换: handler 只返回业务化 browser 请求；executor 创建 browser `task_execution`，并在 browser 终态后把 browser result 引用、FastMoss cookie cache metadata 或失败证据写回原 row job。
- Runtime DB 只保存最小运行事实。workflow contract 不应依赖 `after_browser_candidates`、`fallback_source_job_id` 或类似派生字段推进流程；这些信息如需观测只能作为冗余日志，不作为调度判断事实源。
- Browser `task_execution` 成功只证明浏览器能力完成，不等于业务实体完成。workflow 必须按 fallback 类型处理 browser result，并最终由行级主 job 或等价行级收敛步骤产出 `row_status` / entity status。
- Browser fallback 失败时，只终结当前实体 / 当前行 pipeline 为 `failed` 或按业务允许的 `partial_success`，然后 workflow 才能继续后续实体；不得把 browser 失败直接吞掉后仍把当前行计为成功。
- 父任务进入 `current_stage=ready_for_summary,status=pending` 前，必须确认不存在 waiting row job、active browser execution，或仍未形成最终 `result_status` 的行级主 job。summary gate 只看终态业务结果，不看派生 candidate 数量，也不能只汇总 `api_worker_job` / `task_execution` 的 runtime status。
...
Browser fallback 有两类语义，workflow contract 必须明确是哪一类:
...
- browser fallback 可以作为 child `task_execution` 独立占用 browser profile，但必须由当前行级主 job 创建、等待并按引用消费结果；消费完成前，该行不是 terminal。
- 对严格行级串行 workflow，当前 waiting row job 就是唯一 fallback 待处理事实；browser child 的创建、结果回写和原 row job 重新置为 `pending` 由 executor/runtime 完成，handler 不承担跨 job 数据交换。
- 默认 `row_pipeline_concurrency` 应为 `1`，尤其是客户可见的飞书表采集、补全和写回流程。只有 workflow contract 明确声明 bounded concurrency、幂等边界、FIFO / lane 规则和 summary gate 后，才允许同批多行并行推进。
- 当 `row_pipeline_concurrency=1` 时，当前行从 request、browser fallback、media/FastMoss、Fact DB 到 Feishu writeback 形成最终行级结果后，executor 才能派发下一行。实现上可以释放 worker 进程去处理 browser child execution，但业务 cursor 不能越过当前行。
- 当 workflow 允许 bounded concurrency 时，summary gate 仍必须等待所有 active 行级 pipeline 的最终业务结果；不能只按 child `api_worker_job` / `task_execution` 均为 `success` 判断父任务完成。
...
- 队列资源占用不得随内部步骤数膨胀；同一行的 TikTok request、必要 browser fallback、media sync、FastMoss、Fact DB、飞书写回必须绑定到同一业务单元。
- Runtime 证据应能直接回答“这条业务记录本次刷新最终到了哪一步、结果是什么”，而不是要求跨多个 sibling job 拼接。
- 正确修正方式不是增加更多阶段级 job，而是回到行级 pipeline: 一条候选记录只创建一个主 job，内部串行完成业务采集链路。
...
`orchestrate_*` / `run_*_workflow` / `run_sync_*` 这类名称表达 workflow 编排入口，不是 handler。它们不能作为 `handler_code`、handler registry key、job handler 文件名或目标 handler contract 出现；目标文档只记录当前正式 handler / job 映射。
...
api job:
  ...
  如果明确需要浏览器，生成 browser execution，并把当前实体置为等待 fallback 的中间态
...
reconciler:
  将 browser result 回灌到原实体 / 原行 pipeline
  继续后续业务步骤并产出最终实体状态
```
