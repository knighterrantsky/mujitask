...
- 父任务进入 `current_stage=ready_for_summary,status=pending` 前，必须确认不存在未处理的 wait/fallback signal、active browser execution，或仍未形成最终 `result_status` 的行级主 job。summary 只能汇总业务终态结果，不能只汇总 `api_worker_job` / `task_execution` 的 runtime status。
