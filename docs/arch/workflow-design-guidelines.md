...
- Browser fallback 是物理上的独立 `task_execution`，但在业务语义上仍属于触发它的当前实体 / 当前行 pipeline。`fallback_required` 只能表示“等待 browser 或等待 browser 结果回灌”的中间态，不能作为行级成功、父任务成功或 summary 终态计数。
- Browser `task_execution` 成功只证明浏览器能力完成，不等于业务实体完成。workflow 必须把 browser result resume 回原行级主 job 或等价的行级收敛步骤，由该步骤继续执行 media sync、FastMoss、Fact DB、Feishu writeback 等后续链路，并产出最终 `row_status` / entity status。
...
- 父任务进入 `ready_for_summary` 前，必须确认不存在未处理的 `fallback_required`、未回灌的 browser success、active browser execution 或 active resume job。summary 只能汇总业务终态结果，不能只汇总 `api_worker_job` / `task_execution` 的 runtime status。
...
- browser fallback 可以作为 child `task_execution` 独立占用 browser profile，但必须由当前行级主 job 或该 workflow 的行级 resume stage 创建、等待并回收结果；回收完成前，该行不是 terminal。
...
- 当 `row_pipeline_concurrency=1` 时，当前行从 request、browser fallback、resume、media/FastMoss、Fact DB 到 Feishu writeback 形成最终行级结果后，executor 才能派发下一行。实现上可以释放 worker 进程去处理 browser child execution，但业务 cursor 不能越过当前行。
