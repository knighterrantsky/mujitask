...
- 持久化引用: `artifact_object` id、object storage URI/key、raw snapshot ref、Fact DB entity key、digest、byte size、content type。
...
- 需要按行 fan-out 的 source rows 只能保存 compact row: record id、业务 key、product/creator identity、必要状态字段和 artifact/source snapshot ref。完整飞书行必须进入 artifact/object storage。
- Handler 返回值写入 Runtime 前必须先裁剪为小型结构化 handoff；完整事实、raw evidence 和媒体内容应先写 Fact DB 或对象存储，再把引用写回 Runtime。
