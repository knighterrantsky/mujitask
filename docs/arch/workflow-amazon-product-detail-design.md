...
- [长期业务对象存储机器契约](../../contracts/facts/durable-business-object-storage.yaml)
...
| 对象存储 | 复用现有 MinIO bucket；默认拒绝，Amazon 只准入商品媒体、normalized capture 和 blocked 受控截图 |
...
- Fact DB 保存可重复采集、可追溯、可查询的 Amazon 主体、快照、Offer、变体、BSR、媒体和标准化 capture 索引。
...
flowchart LR
    ...
    Browser --> Local["Local artifact_root<br/>HTML / page-data / diagnostics"]
    Browser --> CaptureStore["MinIO<br/>normalized capture / blocked evidence"]
    CaptureStore --> CaptureRef["紧凑业务对象 refs"]
    ...
    Media --> ObjectStore["MinIO<br/>Amazon 商品媒体"]
...
6. 在本地生成解析所需 HTML、受控数据片段和诊断截图；这些文件不进入 MinIO。
7. 执行分层字段解析。
8. 生成标准化 capture JSON 并上传对象存储。
9. Browser handler 只返回身份、字段覆盖率、状态、`normalized_capture_ref`，以及 blocked/captcha 时的受控截图引用。
...
字段全部丢弃。净化后的可选片段只在内存或本地临时文件中参与解析，允许纳入
`normalized_capture` 的字段白名单，但不得作为独立 `page-data.json` / `network_data`
对象上传 MinIO，也不得成为 Runtime 跨进程依赖。
...
新生成的成功 capture 中 `artifact_refs` 必须为空；HTML、network data、page-data 或普通截图
旧引用不再读取，也不执行修复、回填或迁移。Browser/API 跨进程交接以 capture
对象外层的 `normalized_capture_ref` 为准。
...
sequenceDiagram
    ...
    participant Store as MinIO allowlisted objects
    ...
    Browser->>Store: write normalized capture; blocked only writes governed screenshot
...
| `amazon_product_browser_fetch` | `browser_worker` | `task_execution` | 访问并解析一个 ASIN 页面；成功生成 normalized capture，blocked/captcha 生成受控截图 |
...
- MinIO endpoint、bucket credential。
...
- success / partial success 时的 `normalized_capture_ref`；blocked/captcha/access-blocked 不得返回该引用。
- blocked/captcha/access-blocked 时只返回受控 screenshot ref；成功结果不得返回 HTML、page-data、network-data 或普通截图的持久引用。
- success / partial success 时的受控 `media_source_refs`；blocked/captcha/access-blocked 不得返回或物化商品媒体。每项仅含已按当前图库项绑定到高清资产、已去除 Amazon CDN 尺寸/裁剪/质量变换段及 query/fragment 的 Amazon CDN URL、
...
其字段和值都按 workflow machine contract 投影。API persist job 通过完整
`bucket + object_key + content_digest` 引用直接从 MinIO 读取 capture，并消费上述受控媒体来源
引用，不经 Runtime DB 传递 capture、HTML、临时路径或媒体正文，也不优先/回退读取 Browser
进程的本地文件。
...
- 新写入和读取的 `capture_kind` 只允许 `normalized_capture`；旧 `html`、`network_data`、
  `screenshot` 引用直接拒绝，不执行兼容读取、修复、回填或迁移。
- `bucket`、`object_key`、`content_digest`、`content_type`。
- `request_id`、`execution_id`、`run_id`。
- `sanitization_status`。
- `collected_at`、`created_at`。

数据库只保存 normalized capture 的索引、摘要和 digest，不保存 HTML、page-data、network data、
截图 body 或本地临时路径。blocked/captcha 截图属于终态业务审计证据，不进入成功商品事实事务。
...
首期不新建 bucket。继续使用现有 `artifact_bucket`，但该配置名只为兼容；生产 MinIO
默认拒绝写入，Amazon 仅允许以下长期业务对象:

| 内容 | 推荐 object key |
| --- | --- |
| 标准化 capture | `<env>/raw-captures/amazon/us/<asin>/<yyyy>/<mm>/<dd>/<run_id>/<sha256>/normalized.json` |
| blocked/captcha/access-blocked 截图 | `<env>/raw-captures/amazon/us/<asin>/<yyyy>/<mm>/<dd>/<run_id>/<sha256>/page.png` |
| 商品媒体 | `<env>/product-media/amazon/us/<asin>/<media_role>/<sha256>.<ext>` |

`raw-captures` 是兼容 prefix 名称，不代表可以持久化任意 raw 文件。成功页 HTML、page-data、
network data、普通/成功截图、日志、state dump 和临时下载禁止进入 MinIO。它们只能短期留在
Browser 进程本地 `artifact_root`，不能成为 API persist 的输入。

标准化 capture 写入 `amazon_raw_captures`；业务媒体写入 `amazon_media_assets` 与
`amazon_product_media_assets`。blocked 截图由 Runtime 建立受控业务审计索引，但不进入成功
商品事实事务。

### 11.2 持久化要求

- 成功或 partial success 只要求 `normalized_capture.json`；HTML、page-data、network data 和
  成功截图不得作为成功前置条件或独立持久对象。
- 每个持久 ref 必须携带完整 `bucket + object_key + content_digest`，并绑定当前
  `request_id`、同一个来源 Browser `execution_id` 和当前
  ...
  从 MinIO 读取实际对象字节并复算 digest，不能信任引用 metadata 或 Browser 本地临时文件。
- `local_path` / `source_path` 不是持久坐标。API persist 不得优先读取或 fallback 到 Browser
  临时文件；缺少 bucket、object key 或 digest 时必须拒绝。
...
- `blocked`、`captcha_required` 或 `access_blocked` 在首次 Runtime 写入前必须包含按同一坐标规则
  校验通过的 screenshot business evidence；该例外不授权上传 HTML、page-data、network data
  或其他截图。预导航配置或连接失败没有页面证据时不套用该规则。
...
- `blocked`、`captcha_required` 和 `access_blocked` 必须有截图；其他 failed/身份不一致不上传
  页面截图。任何允许的截图仍必须通过同一坐标、内容类型、大小和 digest 校验。
...
- normalized capture、blocked 截图和商品媒体分别使用受控读取上限；本地 HTML/page-data 也必须
  在 Browser 进程内有独立解析上限，但不因此获得 MinIO 准入。
- 正式 workflow 要求 `requires_fact_db=true` 和 `requires_object_storage=true`。
- 缺少 Fact DB、MinIO provider、bucket 或 credential 时 submit preflight 失败。
...
| --- | --- |
| 标准化 capture | 365 天 |
| blocked/captcha/access-blocked 截图 | 180 天 |
| 商品媒体 | 365 天或商品业务存续期 |
| 本地 HTML/page-data/普通截图/日志 | 短周期 best-effort 清理，不进入 MinIO lifecycle |

旧 HTML、network data、普通截图和 Runtime artifact 对象停止新增，按历史 lifecycle 清理；
不得重新归类为长期业务对象。只有当 Amazon 数据需要独立 IAM、独立地域、独立合规保留或
独立成本核算时，才拆分新 bucket。拆 bucket 必须更新 Storage contract、部署配置和回滚方案。
...
| `access_blocked` | 否 | 只保存受控 blocked 截图，HTML 留在本地短期诊断，状态 blocked |
...
- 网络数据只允许把商品字段 allowlist 合并进 `normalized_capture`；独立 network/page-data
  对象不得上传 MinIO。
- raw HTML 和普通截图按本地敏感诊断文件管理；只有 blocked/captcha/access-blocked 的受控截图
  可以进入私有 bucket，并按业务审计 retention 管理。
...
| 长期业务对象引用交接 | normalized capture 写 MinIO，Runtime 只传完整 bucket/object/digest ref；HTML/page-data 保持本地 | Amazon browser handler + existing object-storage transport |
...
- Projection 不执行网络访问。
- Runtime result 不包含 HTML、完整 capture、cookie、图片 body 或本地临时路径。
- 成功结果只包含 `normalized_capture_ref`，不包含 HTML/page-data/network-data/成功截图持久引用。
- blocked/captcha 只允许一类受控截图证据，不允许借终态上传其他诊断文件。
...
2. 生成并持久化 normalized capture；HTML/page-data/network data 只在本地参与解析。
...
- 缺 Fact DB、MinIO 或 browser profile 时 fail fast。
...
- navigation、parse、business-object、media、fact、Feishu 各阶段耗时。
- 字段覆盖率。
- 准入业务对象/media 数量。
- 最终业务状态和脱敏错误码。
...
5. Browser 成功时只持久化 normalized capture，blocked/captcha 时只持久化受控截图；HTML、page-data、network data 和普通截图保持本地。API Worker 负责媒体、Fact DB 和飞书副作用。
6. Fact DB 使用同一实例中的 `amazon_*` 独立表；首期不拆数据库。
7. MinIO 复用同 bucket，但采用默认拒绝白名单，只保存 Amazon 商品媒体、normalized capture 和 blocked 受控截图；首期不新建 bucket。
8. 飞书是业务入口和最新投影，Amazon Fact DB 是历史事实来源。
9. blocked/captcha 不自动绕过；只受控保留必要截图，不持久化 HTML 或通用页面 dump。
