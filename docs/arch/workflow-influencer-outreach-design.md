...
| 字段 | 写入条件 | 来源 | 覆盖策略 |
| --- | --- | --- | --- |
| `视频链接` | 有视频且最高播放视频 URL 与当前值不同 | SKU+达人最高播放视频 | 可覆盖 |
| `视频发布时间` | 有视频且飞书原字段为空 | SKU+达人最早发布视频时间 | 只补空，不覆盖 |
| `检查时间` | 原 `视频链接` 为空且本次检查完成 | task 触发日期 | 系统覆盖 |
| `播放量` | 目标视频 overview 全部成功且聚合值变化 | SKU+达人全部视频播放量总和 | 系统覆盖 |
| `视频数量` | 数据库已知视频数量变化 | SKU+达人全部已知视频数量 | 系统覆盖 |
| `更新时间` | 本次实际写入任一字段 | task 触发日期 | 系统覆盖 |

写回规则：

- 无 `视频链接` 行：完成检查后写 `检查时间`；找到视频时同时按需写 `视频链接`、`视频发布时间`、`播放量`、`视频数量`、`更新时间`。
- 已有 `视频链接` 行：不更新 `检查时间`；按需刷新 `播放量`、`视频数量`、最高播放 `视频链接` 和 `更新时间`；不更新已有 `视频发布时间`。
