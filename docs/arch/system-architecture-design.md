...
| 文件存储 | MinIO + 本地 `artifact_root` | MinIO 只保存机器契约准入的长期业务对象；HTML、普通截图、raw JSON、stdout 和运行产物只保存在本地，Runtime DB 可保存短期索引 |
...
| Persistence / Object Storage / Media | `media_asset_sync` | 仅物化机器契约显式允许的长期业务媒体；下载和转换临时文件留在本地，通用截图、HTML、raw JSON 与 Runtime artifact 禁止转存 MinIO |
...
flowchart TD
    ...
    Infra --> Obj["MinIO<br/>allowlisted long-term business objects"]
    Infra --> Local["Local artifact root<br/>runtime diagnostics"]

    RDB --> ArtifactIndex["artifact_object index"]
    ArtifactIndex --> Local
```

S3 / local object-store provider 可以保留既有 transport 或测试兼容，但不构成生产长期业务对象的
成功模式。生产白名单对象写 MinIO，运行诊断文件写本地 `artifact_root`。
...
| MinIO Object Store | 机器契约显式允许的长期业务媒体、Amazon normalized capture 和受控 blocked evidence screenshot | 通用截图、HTML、raw JSON、stdout、state dump、临时文件、执行状态唯一真相 |
| Local artifact root | 日志、HTML、普通截图、page/network data、raw 响应诊断、state dump 等短期运行文件 | 长期业务事实、跨进程业务输入 |
...
flowchart TD
    ...
    J --> M["Capability: media sync<br/>allowlisted business objects only"]
