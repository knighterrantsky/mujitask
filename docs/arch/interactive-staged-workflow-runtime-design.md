...
- 本文统一使用 `business_data_ref` 表达跨节点的受控业务数据引用；它优先解析到 Fact DB 记录。它不等于 MinIO object ref，也不授权把候选集、详情包、报告、HTML、日志或其他通用大对象写入 MinIO。
- 跨节点复用的结构化数据必须先进入 Fact DB 或另行通过机器契约准入的业务数据存储；本地 Runtime artifact 只用于短期排障，不得成为跨进程输入。
- 若未来某个字节对象需要 MinIO 持久化，必须先进入 `contracts/facts/durable-business-object-storage.yaml` 的长期业务对象白名单，并使用完整 `bucket + object_key + content_digest`。
...
- 输入输出标准化: 每个节点声明标准输入、标准输出、受控业务数据引用和错误外壳。
...
- 不把大对象塞进 Runtime DB result。可复用结构化数据进入 Fact DB；白名单外字节只留本地短期 artifact，不能通过通用 Object Store 绕开准入。
...
| Business Data Ref | 用 Fact DB ID 或准入长期业务对象引用传递可复用结果 | 避免 Runtime DB 保存大 JSON，同时不把通用数据集变成 MinIO artifact |
...
output_schema:
  ...
  properties:
    candidate_set_ref:
      type: business_data_ref
      data_type: product_candidate_set
    candidate_count:
      type: integer
    preview_rows:
      type: array
...
| `business_data_refs` | 否 | Fact DB ID 或准入长期业务对象引用 | 避免 Runtime DB 保存大 JSON，同时执行存储准入 |
...
```json
{
  "status": "success",
  "output": {
    "candidate_set_ref": "fact-set:product-candidates:req_123:stage_001",
    "candidate_count": 86,
    "qualified_count": 37
  },
  "business_data_refs": [
    {
      "data_ref": "fact-set:product-candidates:req_123:stage_001",
      "data_type": "product_candidate_set",
...
## 6. 能力三: Business Data Ref

### 6.1 目的

Business Data Ref 是跨节点业务数据引用协议。Runtime DB 只保存引用、摘要和 preview。
完整候选集、详情包等结构化业务数据必须解析到 Fact DB 或未来另行受控的数据 contract；
图表、报告等字节若未被长期业务对象白名单准入，只能作为本地短期 artifact，不能供下游
节点跨进程读取，也不能写入 MinIO。
...
- 输入输出标准化: 节点之间传标准引用。
- 输出组合化: final result 聚合多个 business data ref。
- 能力目录开放给 Agent: Agent 可以基于 `data_type` 判断哪些可视化或写回能力可用。

### 6.3 必要字段

| 字段 | 必须 | 说明 | 必要性 |
| --- | --- | --- | --- |
| `data_ref` | 是 | 稳定业务数据 ID | 下游节点和审批据此读取 Fact DB 或受控数据记录 |
| `data_type` | 是 | 数据类型 | Compiler 判断 input/output 是否兼容 |
| `schema_ref` | 是 | 产物结构引用 | 下游节点知道如何解析 |
| `preview_json` | 是 | 小型预览 | Outbox 和 Agent 展示需要，避免读取大对象 |
| `storage_binding` | 是 | 受控存储类型和记录 ID | 默认为 Fact DB；MinIO 只允许白名单对象且必须携带完整持久引用 |
...
```json
{
  ...
  "storage_binding": {
    "kind": "fact_db",
    "record_id": "product-candidate-set:req_123:stage_001"
  },
  "metadata_json": {
...
| `input_data_ref` | 是 | 审批对象 | 用户看到的 Fact 数据集或已准入长期业务对象 |
...
```json
{
  ...
  "approval_type": "search_candidate_review",
  "status": "pending",
  "input_data_ref": "fact-set:product-candidates:req_123:stage_001",
  "allowed_decisions": [
    "continue_selected",
...
| 字段 | 必须 | 说明 | 必要性 |
| --- | --- | --- | --- |
| `side_effect_type` | 是 | 副作用类型 | 区分 external read、writeback、allowlisted business-object write、fact upsert |
...
| 节点 | 标准输出 |
| --- | --- |
| `prepare_search_spec` | `canonical_search_spec`（小型结构化值） |
| `fastmoss_product_search` | `candidate_set_ref`（Fact DB） |
| `candidate_review` | `selected_candidate_set_ref`（Fact DB）, `approval_decision_id` |
| `selected_product_detail_collect` | `product_fact_set_ref`（Fact DB） |
| `detail_review` | `approved_product_fact_set_ref`（Fact DB）, `approval_decision_id` |
| `fact_bundle_upsert` | `fact_write_summary`（小型结构化值） |
| `feishu_selection_writeback` | `feishu_write_summary`（小型结构化值） |
| `task_summary` | `summary`（小型结构化值） |
...
```json
{
  ...
  "outputs": {
    "candidate_set_ref": "fact-set:product-candidates:req_123:stage_001",
    "selected_candidate_set_ref": "fact-set:selected-products:req_123:stage_002",
    "product_fact_set_ref": "fact-set:product-details:req_123:stage_003",
    "fact_write_summary": {"persisted_count": 12},
    "feishu_write_summary": {"written_count": 12}
  },
  "summary": {
    "candidate_count": 86,
    "selected_count": 12,
...
| Business Data Ref | Fact DB record/set ID；白名单字节对象才使用完整 MinIO ref | 不复用本地 `artifact_object` 作为跨节点输入 |
| Outbox | `notification_outbox` | 用 `ref_type/ref_id` 指向 approval 或 node run |

最小落地顺序:

```text
1. 定义 Node Spec / Result Envelope / Business Data Ref contract
2. 定义 WorkflowPlanSpec 和 Compiler 校验规则
3. 增加 Node Run / Stage Run 状态源
4. 增加 Approval Request 状态源
5. 扩展 business data ref / outbox 与 node run、approval 的关联
```
...
- 大型结构化结果先写 Fact DB，再通过 Business Data Ref 传递；白名单外字节不得跨节点。
- 声明 worker 类型。
- 声明副作用。
- 声明幂等键。
- 声明超时和重试策略。
...
- 所有大型结构化数据通过 Fact DB Business Data Ref 传递；字节对象只有经长期业务对象白名单准入后才可跨节点。
