...
### 9.13 2026-09-13 新增请求签名 `fm-sig`

本次真实页面加载的 `/_next/static/chunks/29251-88344b67040381e5.js`
中，模块 `961261` 为业务请求同时生成 `fm-sign` 和 `fm-sig`，后者调用模块
`877830` 注册的 `globalThis.__SIG__.gen`。这修正了 9.9 节只记录旧签名的覆盖范围。

- 输入为请求路径和按键排序的表单编码 query，包含本次 `_time`、`cnonce`；另传 `source`。
- 空格编码为 `+`，`~` 编码为 `%7E`。每次更换参数都必须重新生成两种签名。
- 商品关联视频 `/api/goods/v3/video` 同样需要两种签名；`list_product_videos` 的历史 `sign_request=False` 例外必须移除，否则通用 HTTP 复验成功后，真实来源重试仍会发出未签名请求。
- `fm-sig` 是 64 位小写字母数字串。当前算法可在纯 HTTP 客户端中计算，无须浏览器或保存验证码 ticket。
- 前端验证成功会重新加载页面，并由请求拦截器为新请求生成签名；不能复制一个旧 `fm-sig` 给新 query。

同一失败达人对象的受限实测：验证码确认 `is_ok=true` 后，浏览器及 Python 原样
重放均为 `200`。只更新旧签名和 nonce 时重新触发 `MSG_SAFE_0001`，原成功请求
随后也失效。补上对应新 query 的 `fm-sig` 后，现有客户端保持默认 UA、Cookie
构造及其他请求头不变，连续三次返回 `200`；浏览器原请求和页面重新加载后的
业务请求也均为 `200`。因此，缺失新签名是本次已复现的直接故障；Cookie 导出
本身不是这个样本的故障原因。该结论不等于所有对象都已通过业务验收。

签名由现有 `infrastructure/fastmoss/http_session.py` 负责。使用前端生成的固定
无凭据测试向量核对算法，并在网络尝试时使用该次实际参数生成签名；不记录签名值。
