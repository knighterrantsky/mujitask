...
FLOW_PACKAGE = FLOWS / "competitor_row_refresh"
...
def test_competitor_row_refresh_is_package_not_giant_file() -> None:
    assert FLOW_PACKAGE.is_dir()
    assert not (FLOWS / "competitor_row_refresh.py").exists()
    assert (FLOW_PACKAGE / "__init__.py").is_file()
    assert _is_empty_or_docstring_only(FLOW_PACKAGE / "__init__.py")


def test_competitor_row_refresh_package_shape() -> None:
...
def test_competitor_row_refresh_has_no_dumping_ground_modules() -> None:
    found = [
...
def test_competitor_row_refresh_orchestrator_stays_thin() -> None:
