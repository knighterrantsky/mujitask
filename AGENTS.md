...
用户通常会在 Codex app 的仓库根目录直接输入自然语言需求。不要要求用户记住文档路径、目录结构或 contract 文件名。
...
然后根据 `contracts/codex/task-routing.yaml` 选择最小上下文。默认先读取 `docs/dev/rewrite-state.yaml`、相关 domain README、相关 contract、当前源码和当前测试；只有任务触发条件需要时，才展开长 business / arch 文档。
