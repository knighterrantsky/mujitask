...
涉及创建、切换、重命名或发布分支前，Codex 必须先读取 `docs/dev/git-workflow.md`；涉及发布时还必须读取 `docs/ops/release-flow.md`。

分支命名以项目规范为准，不使用 Codex app 默认 `codex/` 前缀覆盖项目规范。日常开发统一使用:
...
创建或重命名分支后，必须用当前分支查询命令确认实际分支名，再继续实现或声明结果。
