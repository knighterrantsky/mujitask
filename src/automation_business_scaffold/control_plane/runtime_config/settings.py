...
INFLUENCER_MONITOR_TASK_CODE = "monitor_tk_influencers"
...
def build_runtime_settings(params: dict[str, Any]) -> RuntimeExecutionSettings:
    ...
    worker_id = str(
        params.get("execution_worker_id")
        or defaults.worker_id
        or f"{hostname}:{int(time.time())}"
    )
    return RuntimeExecutionSettings(
        db_url=str(params.get("execution_control_db_url") or defaults.db_url).strip(),
        worker_id=worker_id,
        requested_by=str(params.get("requested_by") or defaults.requested_by or "local-cli").strip()
        or "local-cli",
        lease_seconds=max(
            float(params.get("execution_lease_seconds") or defaults.lease_seconds or 120.0),
            5.0,
        ),
        heartbeat_interval_seconds=max(
            float(
                params.get("execution_heartbeat_interval_seconds")
                or defaults.heartbeat_interval_seconds
                or 10.0
            ),
            0.2,
        ),
        poll_interval_seconds=max(
            float(
                params.get("execution_control_poll_interval_seconds")
                or defaults.poll_interval_seconds
                or 1.0
            ),
            0.05,
        ),
