...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="automation-business-scaffold-phase1-executor",
        description="Run the Phase 1 top-level executor for refresh_current_competitor_table.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        with contextlib.redirect_stdout(sys.stderr):
            payload = (
                execute_phase1_executor_once(params)
                if args.once
                else run_phase1_executor_daemon(params)
            )
