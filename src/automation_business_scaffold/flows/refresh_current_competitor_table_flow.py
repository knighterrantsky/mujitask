...
def _build_product_canonical_key(item: dict[str, Any]) -> str:
...
def _build_product_snapshot_facts(item: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _build_snapshot_date(item: dict[str, Any], *, collected_at: float) -> str:
...
def _build_binding_target_space(table_url: str) -> str:
    try:
...
def _persist_product_entity_snapshot(
    *,
    store: Phase1RuntimeStore,
    execution: Any,
    ...
    if not item:
        return {}
    canonical_key = _build_product_canonical_key(item)
    ...
    target_space = _build_binding_target_space(table_url)
    ...
    facts = _build_product_snapshot_facts(item)
    ...
    if baseline_snapshot is not None:
        ...
        diff = _build_snapshot_diff(baseline_snapshot.facts, facts)
    snapshot = store.create_entity_snapshot(
        entity_id=entity.entity_id,
        snapshot_date=_build_snapshot_date(item, collected_at=collected_at),
    ...
    if isinstance(items, list):
        ...
        for raw_item in items:
            if not isinstance(raw_item, dict):
                ...
                continue
            if str(raw_item.get("record_id", "") or "").strip() == str(item.get("record_id", "") or "").strip():
    ...
    for execution in executions:
        ...
        if isinstance(snapshot_payload, dict):
            ...
            if snapshot_id and snapshot_id not in seen_snapshot_ids:
                ...
                entity_snapshots.append(snapshot_payload)
        ...
        with stdout_path.open("w", encoding="utf-8") as stdout_handle:
            ...
            with contextlib.redirect_stdout(tee_stdout), contextlib.redirect_stderr(tee_stderr):
                ...
                if terminal_status == "success":
                    try:
                        _persist_product_entity_snapshot(
                            store=store,
                            execution=execution,
                            result_payload=result_payload,
                        )
