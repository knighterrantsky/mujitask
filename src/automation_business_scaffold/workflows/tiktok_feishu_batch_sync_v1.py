...
def build_tiktok_feishu_batch_sync_workflow(*, run_mode: str = "draft") -> WorkflowSpec:
    return WorkflowSpec(
        ...
        steps=[
            ...
            StepDefinition(
                step_id="collect_tiktok_stage1",
                action=StepAction(type="collect_tiktok_stage1"),
            ...
            StepDefinition(
                ...
                action=StepAction(type="upload_artifacts"),
                effects=["upload"],
                preconditions=["step_output_exists:collect_tiktok_stage1.items"],
                postconditions=["result_data_exists:items"],
                outputs=["items"],
                artifacts={"state_dump": True},
            ),
            StepDefinition(
                step_id="write_back_rows",
                action=StepAction(type="write_back_rows"),
                effects=["write"],
                preconditions=["step_output_exists:upload_artifacts.items"],
                postconditions=["result_data_exists:items"],
                outputs=["items"],
                artifacts={"state_dump": True},
            ),
            StepDefinition(
                ...
                preconditions=[
                    ...
                    "step_output_exists:write_back_rows.items",
                ],
